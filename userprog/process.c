...
__do_fork (void *aux) {
	...
	struct intr_frame *parent_if = parent->sys_if;
	bool succ = true;
...
lazy_load_segment (struct page *page, void *aux) {
	...
	struct container *con = (struct container *) aux;
	struct file *file = con->fp;
	off_t ofs = con->offset;
	size_t page_read_bytes = con->page_read_bytes;
	size_t page_zero_bytes = con->page_zero_bytes;
...
load_segment (struct file *file, off_t ofs, uint8_t *upage,
	...
	while (read_bytes > 0 || zero_bytes > 0) {
		...
		con->fp = file;
		con->offset = ofs;
		con->page_read_bytes = page_read_bytes;
		con->page_zero_bytes = page_zero_bytes;
